...
normalY = list(
    map(lambda x: (math.e ** (-(x ** 2) / 2)) / math.sqrt(2 * math.pi), normalX)
)
_sum = 0
normalY2 = []
for y in normalY:
...
class GameEngine:
        ...
        def tryfn(*args, **kwargs):
            try:
                ...
                self.internalPrint(
                    "error",
                    team,
                    f"Team {team}'s execution raised an error:{type(e)} {e}",
                )
    ...
    def registerBot(self, bot, team=None):
        self.competitors.append({"instance": bot, "team": team, "knowsTrue": False})
        if team is not None:
    ...
    def runGame(self):
        ...
        self.internalPrint(
            "minlog",
            "!ml",
            f"{','.join([c['team'] for c in self.competitors])}|{self.gameParameters['meanTrueValue']}|{self.gameParameters['stddevTrueValue']}/",
        )
        self.gameParameters["numPlayers"] = len(self.competitors)
        for c in self.competitors:
            self.callWithTimeout(
                c["team"],
                c["instance"].onGameStart,
        ...
        for self.auctionNumber in range(self.gameParameters["auctionsCount"]):
            ...
            self.currentBid = 1
            self.currentTurn = 0
            self.resettingBidPlayer = random.randint(0, len(self.competitors) - 1)
            self.currentBidPlayer = (self.resettingBidPlayer + 1) % len(
                self.competitors
            )
            self.lastBidPlayer = self.resettingBidPlayer
            self.nPassed = 0
            self.trueValue = int(
                linterp(normalY2, normalX, random.random())
                * self.gameParameters["stddevTrueValue"]
                + self.gameParameters["meanTrueValue"]
            )
            ...
            for t in self.teams:
                if (
                    ...
                    > (1 - self.gameParameters["knownTrueValueProbability"]) * 100
                ):
            ...
            for i, c in enumerate(self.competitors):
                ...
                if not initialised:
                    ...
                    c["knowsTrue"] = False
            self.internalPrint(
                "engine", "engine", f"Starting Auction {self.auctionNumber}"
            )
            self.internalPrint(
            ...
            # Main loop
            while (
                self.nPassed < len(self.competitors)
                and self.currentBid < self.gameParameters["penaltyMax"]
            ):
                self.protoCurrentBid = -1
                self.callWithTimeout(
                    self.competitors[self.currentBidPlayer]["team"],
                ...
                if self.protoCurrentBid != -1:
                    ...
                    self.internalPrint(
                        "engine",
                        "engine",
                        f"competitor {self.currentBidPlayer} made a bid of {self.currentBid}",
                    )
                    self.internalPrint(
                        "minlog", "!ml", f"{self.currentBidPlayer}:{self.currentBid}|"
                    )
                ...
                self.currentBidPlayer = self.currentBidPlayer % len(self.competitors)
                self.nPassed += 1
                self.currentTurn += 1
            ...
            self.internalPrint(
                "engine",
                "engine",
                "Auction {}: Team {} copped it at a price of {}; true value was {}".format(
                    self.auctionNumber,
                    teamindex if teamindex is not None else "[NPC Random]",
                    self.currentBid,
                    self.trueValue,
                ),
            )
            if teamindex is not None:
                self.teams[teamindex]["score"] = (
                    self.teams[teamindex]["score"]
                    + self.trueValue
                    - self.currentBid
                    - (
                        self.gameParameters["knowledgePenalty"]
                        if self.competitors[self.lastBidPlayer]["knowsTrue"]
                        else 0
                    )
                )
            for t in self.teams:
        ...
        for i in self.teams:
            self.internalPrint(
                "engine", "engine", "Team {}: {}".format(i, self.teams[i]["score"])
            )
        return self.teams
    ...
    def reportTeams(self, reportOwnTeam, reportNNPC, reportKnown):
        ...
        if self.nPassed != -1:
            self.internalPrint(
                "error", self.currentPrintingPlayer, f"Bad report! Still in auction."
            )
            return
        ...
        if type(reportOwnTeam) != list:
            ...
            try:
                    ...
                    if self.competitors[int(tm)]["team"] == self.currentPrintingPlayer:
                        if int(tm) != self.currentBidPlayer:
                            protoReportScore += 100 / (
                                self.teams[self.currentPrintingPlayer]["playersInTeam"]
                                - 1
                            )
                            reportingDone = True
                    else:
                ...
                self.internalPrint(
                    "minlog",
                    "!ml",
                    f":o:{','.join(map(lambda i: str(i), reportOwnTeam))}",
                )
            except ValueError:
                self.internalPrint(
                    "error",
            ...
            try:
                ...
                for tm in reportNNPCDict:
                    if self.competitors[int(tm)]["team"] != "NPC":
                        ...
                        reportingDone = True
                self.internalPrint(
                    "minlog", "!ml", f":n:{','.join(map(lambda i: str(i), reportNNPC))}"
                )
            except ValueError:
                self.internalPrint(
                    "error",
        ...
        if type(reportKnown) != list:
            ...
            try:
                ...
                for tm in reportKnownDict:
                    if (
                        self.competitors[int(tm)]["knowsTrue"]
                        and int(tm) != self.currentBidPlayer
                    ):
                        protoReportScore += 100
                        reportingDone = True
                    else:
                ...
                self.internalPrint(
                    "minlog",
                    "!ml",
                    f":k:{','.join(map(lambda i: str(i), reportKnown))}",
                )
            except ValueError:
                self.internalPrint(
                    "error",
        ...
        if (
            reportingDone
            and self.teams[self.currentPrintingPlayer]["protoReportScore"]
            < protoReportScore
        ):
...
class NPCRandomBot:
    ...
    def onMyTurn(self, lastBid):
        ...
        if random.random() < pr:
            self.engine.makeBid(
                math.floor(lastBid + (self.minp * (1 + 2 * random.random())))
            )
